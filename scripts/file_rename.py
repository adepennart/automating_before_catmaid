...
file_keys,file_values=file_find(OV_folder_list, pattern_1, pattern_3)
print(file_keys,file_values)
#hey
...
if change_num < 0:
	filenames=file_values[0]
else:
	filenames=file_sort(file_values[0],-1,True)
print(filenames)
#print(OV_folder_list[0]+"\\"+str(filter(re.compile("^.\D+").match,filenames[0]))+str(int(filter(re.compile("\d+").match,filenames[0]))+change_num)+str(filter(re.compile("\D+$").match,filenames[0])))

#+"\\"+str(int(filter(re.compile("\d+").match,filenames[num])]

for num in range(0,len(filenames)):
	print(num)
	if windows:
	#   filename = loop_fold+"\\"+filename
		os.rename(OV_folder_list[0]+"\\"+filenames[num],OV_folder_list[0]+"\\"+str(filter(re.compile("^.\D+").match,filenames[num]))+str(int(filter(re.compile("\d+").match,filenames[num]))+change_num)+str(filter(re.compile("\D+$").match,filenames[num])))
